...
from ansible.module_utils.vmware import PyVmomi, get_all_objs, vmware_argument_spec, _get_vm_prop, find_obj
...
class VmwareVmFacts(PyVmomi):
    ...
    def get_all_virtual_machines(self):
        ...
        """
        the_vm_folder = self.module.params.get('vm_folder')
        the_folder = None
        if the_vm_folder is not None:
            the_folder = find_obj(self.content, [vim.Folder], the_vm_folder)

        if the_folder is None and the_vm_folder is not None:
            self.module.fail_json(changed=False, msg="VM folder %s not found" % the_vm_folder)

        virtual_machines = get_all_objs(self.content, [vim.VirtualMachine], the_folder)
        _virtual_machines = {}
        ...
        for vm in virtual_machines:
            ...
            virtual_machine = {
                summary.config.name: {
                    ...
                    "folder": summary.config.vmPathName, 
